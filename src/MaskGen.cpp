...
namespace {
    ...
    // gen~ slide: 每 bin 一阶平滑（上升 rise, 下降 fall, 样本口径）
    inline float slide(float prev, float x, float rise, float fall) {
        float c = (x > prev) ? (1.0f / std::max(1.0f, rise))
                             : (1.0f / std::max(1.0f, fall));
...
void MaskGen::processBin(float magHarmRaw, float envRaw, int binIdx,
